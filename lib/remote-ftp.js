...
module.exports = {
    ...
    "keyboardInteractive": false\n\
			...
			'remote-ftp:download-tab': function ( e ) {
				if (!hasProject()) return;

				atom.project.remoteftp.connect();
				
				var local = $( event.target ).find( '[data-path]:eq(0)' ).attr( 'data-path' );
				var remote = atom.project.remoteftp.toRemote( local );

				atom.project.remoteftp.download(remote, false, function (err) {

				});
			},
			
			'remote-ftp:upload-tab': function ( e ) {
				if (!hasProject()) return;
	...
	fileSaved: function (text) {
		...
		if (!atom.project.remoteftp.isConnected() && !atom.config.get('Remote-FTP.autoConnectOnSave'))
			return;
