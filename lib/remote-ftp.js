...
class Main {
  ...
  fileSaved(text) {
    ...
    // .ftpignore filter
    if (self.client.ignore.ignores(local)) return;
    ...
    self.client.upload(local, () => {
      try {
        ...
        const remotePath = Path.dirname(remote).replace(/\\/g, '/');
        const parent = self.client.resolve(remotePath);
