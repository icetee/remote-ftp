...
module.exports = {
	...
	activate: function (state) {
        ...
        // Add event listener to download each file opened
        atom.workspace.onDidOpen(function(event) {

			if (event.uri.substr(0,5) !== 'atom:') {
				var downloadRemote = atom.project.remoteftp.toRemote(event.uri);
	            atom.project.remoteftp.download(downloadRemote, false, function(err){if (!err)console.log('Download complete - ' + downloadRemote);});
			}
		...
		if (hasProject() && atom.project.getDirectories()[0].resolve('.ftpconfig')) {
			FS.exists(atom.project.getDirectories()[0].resolve('.ftpconfig'), function (exists) {
				if (exists)
					...
					if (document.getElementsByClassName('remote-ftp-view')[0])
						document.getElementsByClassName('remote-ftp-view')[0].style.width = '330px';
			});
		}
		...
		atom.commands.add('atom-workspace', {
			...
			'remote-ftp:navigate-to-current': function () {
				if (!hasProject()) return;
