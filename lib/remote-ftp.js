var FS, Path, $, Client, TreeView, AddDialog, MoveDialog, NavigateTo;
		...
		atom.commands.add('atom-workspace', {
			...
			'remote-ftp:upload-selected': function () {
				...
				locals.forEach(function (local) {
					...
					atom.notifications.addSuccess("File(s) Uploaded");
				});
			},

            'remote-ftp:upload-tab': function () {
                if (!hasProject()) return;

                editor = atom.workspace.getActivePaneItem();
                file = editor.buffer.file ? editor.buffer.file : '';
                filePath = file.path ? file.path : '';

                atom.project.remoteftp.upload(filePath, function (err, list) {
                    if (err)
                        return;
			...
			'remote-ftp:sync-with-local': function () {
				...
				locals.forEach(function (local) {
					...
					atom.project.remoteftp.syncLocalRemote(local, function (err) {
						...
						if (remote) {
							atom.notifications.addSuccess("File(s) Synced with Remote");
