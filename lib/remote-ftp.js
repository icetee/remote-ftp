...
module.exports = {
    ...
    activate: function (state) {
        ...
        //atom.project.on('ftp:project-changed', this.projectChanged.bind(this));
    ...
    deactivate: function () {
        ...
        //atom.project.off('ftp:project-changed', this.projectChanged);
        //atom.project.off('ftp:config-changed', this.connect);
    ...
    /*beginWatch: function () {
        this.watcher = chokidar.watch(atom.project.path, {
            ignoreInitial: true,
            //usePolling: true,
            interval: 10
        });

        this.watcher.on('all', function (event, file, stats) {
            file = atom.project.relativize(file);
            var hash = event+file;

            if (this.queueHash.indexOf(hash) == -1) {
                this.queueHash.push(hash);
                this.queueEvent.push([event, file]);
                //this.queueSilent.push(false);
            }
        }.bind(this));

        this.consumer = setInterval(function () {

            if (this.queueHash.length == 0)
                return;

            var hash = this.queueHash.shift(),
                //silent = this.queueSilent.shift() == true,
                t = this.queueEvent.shift(),
                event = t[0],
                file = t[1];

            //if (!silent)
                atom.project.emit('ftp:project-changed', event, file);
        }.bind(this), 100);
    },

    // Stop watching project's files
    endWatch: function () {
        if (this.watcher) {
            this.watcher.close();
            this.watcher = null;
    ...
    // Project has someting new/removed
    projectChanged: function (event, file) {
        console.warn(event, file);
        if (file == '.ftpconfig')
            this.readConfig();
    },*/
