...
class Main {
  ...
  fileSaved(text) {
    ...
    const uploaded_item = atom.workspace.getActiveTextEditor().getFileName();

    self.client.upload(local, (err) => {
      if (atom.config.get('Remote-FTP.notifications.enableTransfer')) {
        if (err) {
          atom.notifications.addError(`Remote FTP: ${uploaded_item} could not upload.`);
        } else {
          atom.notifications.addSuccess(`Remote FTP: ${uploaded_item} uploaded.`);
        }
      }
      try {
