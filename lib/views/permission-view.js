...
import { isNoChangeGroup, isNoChangeOwner, isNotImplemented } from '../notifications';
...
class PermissionView extends View {
  ...
  initialize(params, remotes) {
    ...
    if (typeof params.rights === 'undefined') {
      isNotImplemented('Modification permission is not available for SFTP');
      this.destroy();
      return;
    }
  ...
  checkOwners() {
    ...
    if (groupText !== '') {
      this.ftp.client.site(`CHGRP ${group} ${this.item.remote}`, (response) => {
    ...
    if (ownerText !== '') {
      this.ftp.client.site(`CHOWN ${owner} ${this.item.remote}`, (response) => {
        if (response[0].code !== 200) {
