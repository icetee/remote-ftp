      ...
      if(gutter.name == "line-number"){
        ...
        View.$(atom.views.getView(gutter)).mouseover(function(event){
          ...
          if(target.hasClass('ftpconfig-pk-prompt')){
            ...
            var input = '<label class="ftpconfig-input-label" style="top:' + (event.clientY-23) +'px;left:' + (event.clientX-60) +'px;display:none;">\
          ...
          if(target.hasClass('ftpconfig-watch-prompt')){
            ...
            inappended.change(function (){
              ...
              for (var i = 0; i < pkMarkers.length; i++){
                ...
                if (marker.isValid()){
                  var appendThis = "";
                  for (var j=0; j < inappended[0].files.length; j++){
                    var file = inappended[0].files[j]
                    var escapedPath = file.path.replace(/\\/g, '\\\\');
