...
import { getObject, resizeCursor, elapsedTime } from '../helpers';
...
class TreeView extends ScrollView {
  ...
  resolve(path) {
    const view = $(`.remote-ftp-view [data-path="${path}"]`);

    return view.map(() => $(this).view() || null).get(0);
  }

  getSelected() {
    const views = $('.remote-ftp-view .selected');

    return views.map(() => $(this).view() || null).get();
  }
