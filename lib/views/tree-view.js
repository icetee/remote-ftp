  ...
  initialize(...args) {
	...
	TreeView.prototype.onDrop = function(e) {
		...
		if (entry = e.target.closest('.entry')) {
			...
			projectPath = atom.project.remoteftp.getProjectPath();
			if (initialPath) {
				...
				for (j = 0, len = ref3.length; j < len; j++) {
					file = ref3[j];
					if(newDirectoryPath.slice(-1) == '/'){
						remotePath = newDirectoryPath+file.name;
					} else {
						remotePath = newDirectoryPath+'/'+file.name;
					}
					atom.project.remoteftp.upload(file.path,false,remotePath);
				}
				return results;
			}
		}
