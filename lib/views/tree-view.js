...
module.exports = TreeView = (function (parent) {
	...
	TreeView.prototype.initialize = function (state) {
		...
		// Events
		atom.config.observe('tree-view.showOnRightSide', {callNow: false}, function () {
			self.addClass(atom.config.get('tree-view.showOnRightSide') ? 'panel-right' : 'panel-left');
		});
		atom.config.observe('remote-ftp.hideLocalWhenDisplayed', {callNow: false}, function () {
			self.toggle();
			self.toggle();
		});
		...
		atom.project.remoteftp.on('queue-changed', function () {
			...
			if (atom.project.remoteftp._current)
				queue.push(atom.project.remoteftp._current[0]);
			for (var i = 0, l = atom.project.remoteftp._queue.length; i < l; ++i)
				queue.push(atom.project.remoteftp._queue[i][0]);
			...
			if (queue.length == 0)
				...
				for (var i = 0, l = queue.length; i < l; ++i) {
					var $li = $('<li><progress class="inline-block" />'+ queue[i] +'</li>');
	...
	TreeView.prototype.attach = function () {
		this.localTreeView = $('.tree-view-resizer');
		this.localTreeView.after(this);

		this.addClass(this.localTreeView.is('.panel-left') ? 'panel-left' : 'panel-right');
	}

	TreeView.prototype.detach = function () {
		TreeView.__super__.detach.apply(this, arguments);

		this.localTreeView.show();
	}
	...
	TreeView.prototype.resizeVerticalView = function (e) {
		...
		var delta = e.pageY - this.resizeMouseStart,
			height = Math.max(25, this.resizeHeightStart - delta);
