...
module.exports = TreeView = (function (parent) {
  ...
  TreeView.prototype.initialize = function (state) {
    ...
    atom.config.onDidChange('Remote-FTP.tree.useNewDockIntegration', () => {
    ...
    atom.project.remoteftp.on('queue-changed', () => {
      ...
      for (let i = 0, l = atom.project.remoteftp._queue.length; i < l; ++i) {
  ...
  TreeView.prototype.attach = function () {
    const enableDock = atom.config.get('Remote-FTP.tree.useNewDockIntegration');
