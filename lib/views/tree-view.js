...
module.exports = TreeView = (function (parent) {
  ...
  TreeView.prototype.attach = function () {
    ...
    if (atom.config.get('tree-view.showOnRightSide') && atom.config.get('Remote-FTP.useNewDockIntegration')) {
      ...
      this.panel = atom.workspace.getLeftDock().paneContainer.getActivePane().addItem(this).show();
  ...
  TreeView.prototype.detach = function () {
    ...
    if (this.panel) {
      if (atom.config.get('tree-view.showOnRightSide') && atom.config.get('Remote-FTP.useNewDockIntegration')) {
        atom.workspace.getRightDock().paneContainer.getActivePane().destroyItem(this.panel);
      } else if (!atom.config.get('tree-view.showOnRightSide') && atom.config.get('Remote-FTP.useNewDockIntegration')) {
        atom.workspace.getLeftDock().paneContainer.getActivePane().destroyItem(this.panel);
      } else if (!atom.config.get('Remote-FTP.useNewDockIntegration')) {
        //console.log(1);
        this.panel.destroy();
      }
