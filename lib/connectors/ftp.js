...
import { isEEXIST, isEISDIR, isAlreadyExits } from '../notifications';
import { getObject, separateRemoteItems, splitPaths, simpleSortDepth, sortDepth } from '../helpers';
...
class ConnectorFTP extends Connector {
  ...
  _getFolder(path, recursive, completed, progress) {
    ...
    this.list(npath, recursive, (lError, list) => {
      ...
      list.forEach((item) => { item.depth = splitPaths(item.name).length; });
      list.sort(sortDepth);
  ...
  put(path, completed, progress, remotePath) {
    if (!this._isConnectedApply(completed)) return false;
    ...
    if (FS.isFileSync(path)) {
      ...
      this.ftp.put(path, remote, (err) => {
        ...
        if (err && !fatal) {
          this.mkdir(Path.dirname(remote).replace(/\\/g, '/'), true, () => {
      ...
      // NOTE: Folder
      this.client.traverseTree(path, (list) => {
      ...
      this.ftp.list(dir, false, (errList, list) => {
        if (typeof list !== 'undefined') {
          ...
          if (typeof list !== 'undefined' && folders.map(o => o.name).indexOf(dirName) > -1) {
            if (enableTransfer) isAlreadyExits(dir, 'folder');
