...
import { getObject, separateRemoteItems, splitPaths } from '../helpers';
  ...
  put(path, completed, progress) {
    ...
    if (FS.isFileSync(path)) {
      ...
      this.ftp.put(path, remote, (err) => {
        ...
        if (err && !fatal) {
          this.mkdir(Path.dirname(remote).replace(/\\/g, '/'), true, (err) => {
  ...
  rename(source, dest, completed) {
    ...
    this.ftp.rename(source, dest, (err) => {
      if (err) {
        ...
        FS.rename(this.client.toLocal(source), this.client.toLocal(dest), (err) => {
          tryApply(completed, null, [err]);
        });
      }
    });
