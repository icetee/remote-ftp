...
class ConnectorSFTP extends Connector {
  ...
  connect(info, completed) {
    ...
    self.ssh2.on('keyboard-interactive', (name, instructions, instructionsLang, prompts, finish) => {
      if (typeof self.info.password !== 'undefined') {
        finish([self.info.password]);
      } else {
        atom.notifications.addInfo('Keyboard Interactive is on, but the password is not defined.', {
          description: 'Try turning on <b>"promptForPass": true</b>',
        });
      }
    });
