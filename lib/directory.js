...
module.exports = Directory = (function (parent) {
	...
	Directory.prototype.openPath = function (path) {
				...
				self.folders.forEach(function (folder)
				{
					if (folder.remote === nextPath)
					{
						...
						else
						{
							...
							{
								folder.openPath(path);
								if (document.querySelector('[data-path="' + path + '"]') && document.querySelector('[data-path="' + path + '"]').parentNode.classList[3] === "selected") {
									console.log('downloading');
									client.download(path, false, function(){console.log('complete');});

									Path = require('path')
									client.emit('debug', '<b>' + Path.basename(path) + ' downloaded</b>');
								}
        ...
        if (path.indexOf('.', path.lastIndexOf('/')) > 0) {
            ...
            var parentPath = path.substring(0, path.indexOf(pathNode.basename(path)) - 1);
            parentPath = parentPath.substr(0, parentPath.lastIndexOf('/'))

			if (document.querySelector('[data-path="' + parentPath + '"]'))
                document.querySelector('[data-path="' + parentPath + '"]').parentNode.scrollIntoView();
