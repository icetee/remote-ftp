    ...
    'remote-ftp:download-active': {
      ...
      command() {
        ...
        if (!client.ftpConfigPath) {
          client.readConfig(function (err) {
            if (err !== null) {
              return;
            }
          });
        ...
        var currentPath = atom.workspace.getActivePaneItem().getPath();
        if (currentPath === client.getConfigPath()) return;
        if (client.watch.files.indexOf(currentPath) >= 0) return;
        var download_item = client.toRemote(currentPath);
        client.download(download_item, true, () => {
