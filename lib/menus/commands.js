  ...
  const commands = {
    ...
    'remote-ftp:create-ignore-file': {
      ...
      command() {
        ...
        const fileContents = `# Please note that this is a beta-feature and will only work with simple files and full folder!
# For the time being no support negation rule!
      ...
      command() {
        ...
        locals.forEach((local) => {
          ...
          client.upload(local, (err, list) => {
            if (atom.config.get('Remote-FTP.notifications.enableTransfer')) { attemptedTransfers++; }
            if (err) return;
            if (atom.config.get('Remote-FTP.notifications.enableTransfer')) { successfulTransfers++; }
