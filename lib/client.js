				...
				FS.stat(fileName, function(err, stats) {
					if (err === null) {
						...
						if (stats.isDirectory()) {
							var folder = Path.basename(fileName);
							var watcher = FS.watch(fileName, function(event, file) {

								var path = Path.dirname(fileName);
								// if (event === "rename") {
								// 	atom.notifications.addError("Remote FTP: The folder " + folder + " has either been deleted or renamed, removing watch listener on this folder.");
								// 	self.watch.removeListeners.apply(self, [fileName]);
								// }
								self.watch.queueUpload.apply(self, [path + "/" + folder + "/" + file]);
							});
						}
