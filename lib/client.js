...
module.exports = (function() {
    ...
    this.status = "NOT_CONNECTED"; // Options NOT_CONNECTED, CONNECTING, CONNECTED
	...
	Client.prototype.readConfig = function(callback) {
		...
		FS.readFile(self.ftpConfigPath, 'utf8', function(err, res) {

			if (err) return e(err);

			var data = stripJsonComments(res);
			var json = null;
	...
	Client.prototype.getFilePath = function (relativePath) {
		var self = this;
		var projectPath = self.getProjectPath();
		return Path.resolve(projectPath, relativePath);
	};

	Client.prototype.getProjectPath = function() {
		var self = this;
		var projectPath;
	...
	Client.prototype.getConfigPath = function(projectPath) {
		// var self = this;
		// self.projectPath = null;
		//
		// if(multipleHostsEnabled() === true){ // if multiple hosts option is checked
		// 	self.projectPath = self.getProjectPath();
		// 	return self.projectPath + '/.ftpconfig';
		// }
		//
		// return atom.project.getDirectories()[0].resolve('.ftpconfig');
		var self = this;
        ...
        self.status = "CONNECTING";
          ...
          if(err !== null){
            self.status = "NOT_CONNECTED";
						// NOTE: Remove notification as it will just say there is no ftpconfig if none in directory
				...
				self.connector.list(remote, true, function(err, remotes) {
					...
					self._traverseTree(local, function(locals) {
						...
						var n = function() {
							...
							toRemote = self.toRemote(local.name);
							remote = null;
	...
	Client.prototype.watch = {
		...
		addListeners: function() {
			...
			var dir = multipleHostsEnabled() === true ? self.projectPath : atom.project.getDirectories()[0].getRealPathSync();
