...
module.exports = (function () {
	...
	Client.prototype.connect = function (info) {
		var self = this;
		...
		self.ftp.on('ready', function (msg) {
			self.emit('connected', msg);
			self.root.open();
		});
		self.ftp.on('close', function (msg) {
			self.emit('closed', msg);
		});
		self.ftp.on('end', function (msg) {
			self.emit('ended', msg);
		});
		self.ftp.on('error', function (msg) {
			self.emit('error', msg);
		});
	...
	Client.prototype.upload = function (local, callback) {
		...
		if (fs.isFileSync(local)) {
			self.mkdir(path.dirname(local), true, function (err) {
				if (err) {
					self._next();
					if (typeof callback === 'function')
						callback.apply(null, [err]);
				...
				self.ftp.put(local, remote, function (err) {
					self._next();
					if (typeof callback === 'function')
						callback.apply(null, [err, [{name: local, type: '-'}]]);
				})
			});
		}
