  ...
  class Client extends EventEmitter {
      ...
      FS.readFile(self.ftpConfigPath, 'utf8', (err, res) => {
        ...
        if (json !== null && typeof callback === 'function') {
          ...
          if (ssconfigPath && self.info.protocol === 'sftp') {
            const configPath = Path.normalize(ssconfigPath.replace('~', require('os').homedir()));
    ...
    traverseTree(rootPath, callback) {
      ...
      while (queue.length > 0) {
        ...
        // console.log('existsSync', fs.existsSync(currentPath));

        console.log(fs.existsSync(currentPath));
        console.log(fs.fsyncSync(currentPath));
