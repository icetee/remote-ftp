...
export default class Client {
  ...
  readConfig(callback) {
    ...
    const csonConfig = new File(this.getFilePath(`${self.ftpConfigPath}.cson`));
    ...
    if (csonConfig.existsSync()) {
      ...
      csonConfig.read(true).then((content) => {
        try {
          ...
          atom.notifications.addError(`Could not process \`${self.configFileName}\`.`, {
            detail: e,
            dismissable: false,
          });
        }
    ...
    FS.readFile(self.ftpConfigPath, 'utf8', (err, res) => {
      ...
      let json = null;
      if (validateConfig(data, self.configFileName)) {
        try {
  ...
  downloadTo(remotePath, targetPath, recursive, callback) {
    this.checkIgnore(remotePath);

    if (this.ignoreFilter && this.ignoreFilter.ignores(remotePath)) {
      this._next();
    }
  ...
  uploadTo(local, remote, callback) {
    console.log(remote);
    this.checkIgnore(local);
    if (this.ignoreFilter && this.ignoreFilter.ignores(local)) {
      this._next();
      return;
    }
