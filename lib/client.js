	...
	Client.prototype.connect = function (reconnect) {
		...
		switch (self.info.protocol) {
			case 'ftp':
				...
				break;
			default:
				throw "No `protocol` found in connection credential. Please recreate .ftpconfig file from Packages -> Remote-FTP -> Create config file or with the command `remote-ftp:create-config`.";
		}
