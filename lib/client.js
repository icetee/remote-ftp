  ...
  class Client extends EventEmitter {
    ...
    getProjectPath() {
      ...
      if (multipleHostsEnabled() === true) {
        ...
        projectPath = $currentProject.find('.header span.name')
          .data('path');
      } else {
    ...
    validateConfig(data) {
      try {
        ...
        let lineNumber = -1;
        let regex = /at position ([0-9]+)$/;
        let result = error.message.match(regex);
        if (result && result.length > 0) {
          let cursorPos = parseInt(result[1]);
          // count lines until syntax error position
          let tmp = data.substr(0, cursorPos);
        ...
        atom.workspace.open('.ftpconfig')
          .then((editor) => {
            ...
            editor.getDecorations(decorationConfig)
              .forEach((decoration) => {
                decoration.destroy();
              });

            const range = editor.getBuffer()
              .clipRange([
                [lineNumber, 0],
                [lineNumber, Infinity]
              ]);
    ...
    initIgnoreList() {
      ...
      self.ignorePath = self.getFilePath('.ftpignore');

      if (FS.existsSync(self.ignorePath)) {
        let ignoreFile = FS.readFileSync(self.ignorePath).toString();
        self.ignore = ignore().add(ignoreFile);
      }
    ...
    _traverseTree(path, callback) {
      ...
      let trverseDirs = [path];
      while (trverseDirs.length) {
        ++digg;
        let p = trverseDirs.pop();
        FS.readdir(p, (err, lis) => {
          ...
          lis.forEach((name) => {
            ...
            if (!stats.isFile()) trverseDirs.push(name);
          });
