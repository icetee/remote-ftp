...
module.exports = (function () {
	...
	Client.prototype.upload = function (local, callback) {
		...
		self.onceConnected(function () {
			self._enqueue(function (progress)
			{
				// NOTE - .ftpconfig should never be uploaded for security reasons, regardless of .ftpignore
				var ftpconfig = '.ftpconfig';
				var canUpload = local.indexOf(ftpconfig, local.length - ftpconfig.length) === -1;
					...
					self._traverseTree(local, function (locals) {
						...
						var n = function () {
							...
							if ( ! remote || remote.size != local.size )
							{
								...
								var ftpconfig = '.ftpconfig';
								var canUpload = localName.indexOf(ftpconfig, localName.length - ftpconfig.length) === -1;

								if ( canUpload )
									self.connector.put(localName, function () {
										return n();
									});
								else
									n();
