...
export default (function INIT() {
  class Client extends EventEmitter {
    ...
    checkIgnore(local) {
      ...
      let ignoreBaseName = '.ftpignore',
          ignorePath = self.getFilePath(ignoreBaseName);

      if (typeof self.ignoreFile === "undefined") {
        self.ignoreFile = new File(ignorePath);
      }
      ...
      if (self.ignoreFile.getBaseName() === ignoreBaseName) {
    ...
    doConnect() {
      ...
      self.connector.on('response', (code, text) => {
        if (code === 550 && !/No such file or directory/.test(text)) {
          atom.notifications.addError('Remote FTP: Transfer issue', {
            detail: `${code} ${text}`,
            dismissable: false,
          });
        }
    ...
    watchDebug(isWatching) {
      ...
      if (isWatching) {
        self.on('debug', self.logger);
      } else {
        self.removeListener('debug', self.logger);
      }
    }
          ...
          self.connector.list(remote, true, (err, remotes) => {
            ...
            self.traverseTree(local, (locals) => {
              ...
              for (var i = locals.length - 1; i >= 0; i--) {
                if (typeof self.ignoreFilter === "undefined") break;
